...
def main():
    ...
    while True:
        ...
        if op == 1:
            ...
            while True:
                cota = check_cota(
                    "Introduzca la cota de la pintura. \nDebe tener 4 letras seguido de 4 números. Ejemplo: 'ABDC1234'\n==>"
                )
                alreadyExists = check_list("cota", cota, indice_cotas)

                if alreadyExists == True:
                    print(
                        "Error, la cota ingresada pertenece a otra pintura. Ingrese otra cota nuevamente."
                    )
                    print("")
                else:
                    break
            ...
            while True:
                nombre = (
                    (
                        check_nom(
                            "Introduzca el nombre de la pintura (máx. 10 caracteres)\n==>"
                        )
                    )
                    .lower()
                    .capitalize()
                ...
                if alreadyExists == True:
                    print(
                        "Error, el nombre ingresado pertenece a otra pintura. Por favor, ingrese otro nombre."
                    )
                    print("")
                else:
                    break
            ...
            anho = check_num(
                True, "Introduzca el año de creación de la pintura. Ejemplo '2023'\n==>"
            )
            status = check_status(
                """Indique el estado de la pintura
                                  \n1.- En mantenimiento
                                  \n2.- En exhibición
                                  \n==>"""
            )
        ...
        if op == 2:
            while True:
                op2 = check_op(
                    1,
                    3,
                    """Ingrese la opción de búsqueda que desea realizar:     
                            \n1.- Por cota
                            \n2.- Por nombre
                ...
                if op2 == 1:
                    ...
                    indice_pintura_array = search(
                        "cota", indice_cotas, 0, len(indice_cotas) - 1, cota
                    )
                ...
                if op2 == 2:
                    ...
                    indice_pintura_array = search(
                        "nombre", indice_nombres, 0, len(indice_nombres) - 1, nombre
                    )
                    indice_pintura = indice_pintura_array[0]
                    if indice_pintura != -1:
                        pintura_buscada = pinturas_db[indice_pintura]
            ...
            while True:
                ...
                if op3 == 1:
                    op31 = check_op(
                        1,
                        3,
                        """Ingrese la opción de búsqueda que desea realizar para cambiar el status:     
                                \n1.- Por cota
                                \n2.- Por nombre
        ...
        if op == 4:
            while True:
                op4 = check_op(
                    1,
                    3,
                    """Ingrese la opción de búsqueda que desea realizar para buscar la pintura a borrar:     
                        \n1.- Por cota
                        \n2.- Por nombre
                        \n3.- Volver al menú principal
                        \n-->""",
        ...
        if op == 5:
            op5 = check_op(
                1,
                2,
                """¿Está seguro/a de proseguir con la compactación?:     
                        \n1.- Si
                        \n2.- No
                        \n-->""",
            )
            if op5 == 1:
                ...
                while num < len(pinturas_db):
                    ...
                    cota = picture.cota
                    indice_nombre_array = search(
                        "nombre", indice_nombres, 0, len(indice_nombres) - 1, nombre
                    )
                    nombre_index = indice_nombre_array[1]
                    indice_cota_array = search(
                        "cota", indice_cotas, 0, len(indice_cotas) - 1, cota
                    )
